...
void	*check_dead(void *num)
{
	...
	{
		i=0;
		while (i<philo.num_phil && philo.num_meals[i] < philo.meals)
		{
			if ((current_time() - philo.last_meal[i]) >= philo.time_die)
			{
				pthread_mutex_lock(&philo.dead_lock);
				print_status("died", i, &philo);
				print_status("last meal", current_time() - philo.last_meal[i], &philo);
				philo.dead = 1;
...
void	*routine(void *num)
{
	...
	while ((philo->num_meals)[id]++ < philo->meals)
	{
        ...
        print_status("last meal", philo->last_meal[id] -philo->time_start, philo);
